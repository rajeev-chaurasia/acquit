...
The plugin is the last hop between a proof and a test run. It only applies a
selection-v2 document after verifying, once per session, that the document
describes the tree it is being applied to: schema, mode, graph hash, git repo
root, and a recomputed tree fingerprint all have to line up, and anything
surprising means every test runs. Every reproduction here runs real pytest in
a subprocess so the installed plugin, not a stub, does the deselecting.
"""
...
def git_simple_project(path: Path) -> Path:
    """The simple project inside a real repository, so documents can bind it."""
    repo = new_repo(path)
    write(repo, SIMPLE_TESTS)
    return repo


def test_stale_selection_deselects_nothing_once_the_tree_moves_on(tmp_path: Path) -> None:
    """ADV-FC-1: the tree fingerprint refuses a document for a different tree."""
    ...
    assert skip_paths(read_json(stale)) == ["tests/test_beta.py"]
    ...
    assert skip_paths(read_json(fresh / "selection.json")) == ["tests/test_alpha.py"]
...
def test_stale_selection_is_undetectable_from_the_graph_hash(tmp_path: Path) -> None:
    """The graph hash cannot detect content drift; the tree fingerprint can."""
    repo = two_module_repo(tmp_path / "repo")
    base = commit(repo, "base")
    write(repo, {"alpha.py": "ALPHA = 2\n"})
    alpha_change = commit(repo, "change alpha")
    ...
    assert before["tree"]["fingerprint"] != after["tree"]["fingerprint"]


def test_selection_from_a_different_project_root_deselects_nothing(tmp_path: Path) -> None:
    """ADV-FC-2: a document for one root never deselects under another root."""
    other = git_simple_project(tmp_path / "other")
    ...
    selection = write_json(
        tmp_path / "selection.json", selection_doc(["tests/test_a.py"], tree=other)
    )
...
def test_rootdir_outside_the_collected_tree_still_runs_every_test(tmp_path: Path) -> None:
    """ADV-FC-3: an unrelated rootdir degrades to run-all, never a crash."""
    project = simple_project(tmp_path / "project")
...
def test_missing_selection_under_filterwarnings_error_runs_everything(tmp_path: Path) -> None:
    """ADV-FC-4: the degraded path never signals through the warnings system."""
    project = simple_project(tmp_path / "project")
...
def test_explicitly_requested_test_file_still_runs(tmp_path: Path) -> None:
    """ADV-FC-5: a file named on the command line is never deselected."""
    project = git_simple_project(tmp_path / "project")
    selection = write_json(
        tmp_path / "selection.json", selection_doc(["tests/test_a.py"], tree=project)
    )
...
def test_deeply_nested_selection_json_runs_everything(tmp_path: Path) -> None:
    """ADV-FC-6: any exception while loading, RecursionError included, degrades."""
    project = simple_project(tmp_path / "project")
...
def test_hostile_skip_entry_shapes_never_deselect(tmp_path: Path, shape: str) -> None:
    ...
    selection = write_json(tmp_path / "selection.json", selection_doc([entry], tree=project))
...
def test_non_ascii_skip_entry_never_deselects_another_file(tmp_path: Path) -> None:
    ...
    selection = write_json(
        tmp_path / "selection.json", selection_doc(["tests/test_ünicode.py"], tree=project)
    )
...
def test_deselecting_every_test_is_not_a_failed_run(tmp_path: Path) -> None:
    """ADV-FC-12: a perfect selective result exits 0, not NO_TESTS_COLLECTED."""
    project = git_simple_project(tmp_path / "project")
    selection = write_json(
        tmp_path / "selection.json",
        selection_doc(["tests/test_a.py", "tests/test_b.py"], tree=project),
    )
...
def test_selective_document_with_a_null_graph_hash_is_refused(tmp_path: Path) -> None:
    """A hand-written document without provenance must not deselect anything."""
    project = git_simple_project(tmp_path / "project")
    document = selection_doc(["tests/test_a.py"], graph_hash=None, tree=project)
